...
class Solver(object):
    ...
    def label2onehot(self, labels, dim):
        ...
        out[np.arange(batch_size), labels.long()] = 1  # i열 값을 다 1로 설정
        return out
    ...
    def create_labels(self, c_org, c_dim=5, dataset='CelebA', selected_attrs=None):
        ...
        for i in range(c_dim):
            if dataset == 'CelebA':
                ...
                c_trg = self.label2onehot(torch.ones(c_org.size(0)) * i, c_dim)  # batch_size 크기이며 값이 모두 i인 텐서와 c_dim 전달
